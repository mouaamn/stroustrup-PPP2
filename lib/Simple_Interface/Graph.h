...
#undef minor
	...
	private:
		char v;	// invisible or visible for now
		Fl_Color c;
	};
	...
	public:
		...
		void push_back(T& s) { v.push_back(&s); }						// add a named object
		void push_back(T* p) { v.push_back(p); owned.push_back(p); }	// add an unnamed object

		// subscrpting: read and write access
	...
	// with this type alias, a capturing lambda can't convert to a function pointer
	// on page 533 of PPP2, the author shows a broken code fragment introducting capturing lambdas
	//using Fct = double (*)(double);
	// the fix is:
	using Fct = std::function<double(double)>;
	// but this introduces another issue where we can't provide overloaded functions (e.g. std::cos)
	// what you need to do is wrap an overloaded function call in a lambda:
	//		Function f{ [](double n) { return std::cos(n); }, -10, 11, { 300, 200 } }; 
	...
	struct Axis : Shape {
		...
		Axis(Orientation d, Point xy, int length,
			int nummber_of_notches = 0, string label = "");
	...
	struct Open_polyline : Shape { // open sequence of lines
		// constructors still protected - you can only change access of non-constructors with "using"
		//using Shape::Shape;		   // use Shape's constructors (A.16)
		// an alternative is delegation:
		Open_polyline() {}
		Open_polyline(initializer_list<Point> lst)
			: Shape{ lst }
		{}
		void add(Point p) { Shape::add(p); } // this or "using Shape::add;"
	};

	struct Closed_polyline : Open_polyline { // closed sequence of lines
		using Open_polyline::Open_polyline;	 // use Open_polyline's constructors (A.16)
		void draw_lines() const;
	};
