/*
	...
	Input comes from Token_stream::source through the Token_stream ts.
*/
...
Token Token_stream::get() {
	...
	char ch;
	while (source.get(ch) && isspace(ch))
		if (ch == '\n')
			break;
	if (!source)
		error("Bad input");
	...
	default:
		if (isalpha(ch) || ch == '_') {	// a NAME starts with an alphabet or underscore
			source.putback(ch);
			string s;
			while (source.get(ch) && (isalpha(ch) || isdigit(ch) || ch == '_'))
				s += ch;
			source.putback(ch);
			if (s == DECL_KEY)
				return Token{ LET };
